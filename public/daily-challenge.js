...
export class DailyChallenge {
    ...
    init() {
        ...
        if (savedDate !== today) {
            ...
            // Load saved challenge (guard against missing/corrupted data)
            const saved = localStorage.getItem('dailyChallenge');
            this.currentChallenge = saved ? JSON.parse(saved) : null;
    ...
    completeChallenge() {
        ...
        // Increment user profile counter (needed for ROCKET character unlock)
        if (window.userProfile) {
            window.userProfile.dailyChallengesCompleted = (window.userProfile.dailyChallengesCompleted || 0) + 1;
            try {
                localStorage.setItem('dailyChallengesCompleted', window.userProfile.dailyChallengesCompleted.toString());
            } catch (e) { /* silent */ }
        }
    ...
    updateUI() {
        ...
        if (this.currentChallenge) {
            ...
            descEl.textContent = "No challenge today.";
